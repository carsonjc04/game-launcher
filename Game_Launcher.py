...
def launch_game(game):
    try:
        ...
        if game == "Clicker Game":
            subprocess.Popen(["python3", "clicker_game.py"])  # Launch Clicker Game
        elif game == "Puzzle Game":
            subprocess.Popen(["python3", "puzzle_game.py"])  # Launch Puzzle Game
        elif game == "Adventure Game":
            subprocess.Popen(["python3", "adventure_game.py"])  # Launch Adventure Game
        elif game == "Slime Slayer":
            subprocess.Popen([r"Slime Slayer Version A.5\Slime Slayer.exe"], shell=True)
